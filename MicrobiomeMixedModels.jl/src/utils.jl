...
function get_post(sim, data, param)
    ...
    param_names = Dict{Symbol, Any}(
        ...
        :γ => ["γ[$k, $r]" for k in 1:K, r in 1:q],
        :γ_var => ["γ_var[$bf]" for bf in 1:num_blocking_factors],
        ...
        :τ => ["τ[$l]" for l in 1:L],
        :ν => ["ν"],
        :g => ["g[$l, $r]" for l in 1:L, r in 1:q],
        :g_var => ["g_var[$bf]" for bf in 1:num_blocking_factors],
